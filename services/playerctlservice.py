...
class SimplePlayerctlService(Service):
    ...
    def get_metadata(self, player_name=None):
        ...
        try:
            ...
            artist = self._get_variant_artist(metadata.lookup_value('xesam:artist', None))
            album = self._get_variant_string(metadata.lookup_value('xesam:album', None))
            art_url = self._get_variant_string(metadata.lookup_value('mpris:artUrl', None))
    ...
    # Album art methods (your original logic)
    def create_album_art(self,pix,size=200):
...
#     service = SimplePlayerctlService()
...
#             # Get album art
#             if metadata['art_url']:
#                 pixbuf = service.get_album_art_pixbuf(metadata['art_url'], 200)
#                 if pixbuf:
#                     print(f"Album art loaded: {pixbuf.get_width()}x{pixbuf.get_height()}")
        
#         # Get position and status
#         position = service.get_position()
#         status = service.get_status()
#         print(f"Status: {status}, Position: {position:.1f}s")
