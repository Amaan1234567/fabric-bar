"""
This module defines the AnimatedCircularScale widget, which extends the CircularScale
from the fabric library to add animation to the progress updates.
"""

from functools import partial
from typing import Any

from fabric.widgets.circularscale import CircularScale
from utils.animator import Animator, _cubic_bezier


class AnimatedCircularScale(CircularScale):
    """An animated circular scale."""

    def __init__(self, **kwargs: Any):
        """Initialize the scale with optional parameters."""
        super().__init__(**kwargs)
        self.animator: Animator = (
            Animator(
                # edit the following parameters to customize the animation
                timing_function=partial(_cubic_bezier, 0.34, 1.56, 0.64, 1.0),
                duration=0.4,
                min_value=self.min_value,
                max_value=self.max_value,
                tick_widget=self,
                notify_value=lambda p, *_: self.set_value(p.value),
            )
            .build()
            .play()
            .unwrap()
        )

    def animate_value(self, value: float) -> None:
        """Animate the value of the progress bar."""
        self.animator.pause()
        self.animator.min_value = self.value
        self.animator.max_value = value
        self.animator.play()