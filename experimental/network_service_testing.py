...
class WifiService(Service):
    ...
    def _on_state_changed(
        ...
        logger.debug(
            f"Wifi state changed: {old_state.value_nick} -> {new_state.value_nick}"
        )

        if new_state == NM.DeviceState.ACTIVATED:
            self.emit("enabled")
        elif new_state in (
            NM.DeviceState.DEACTIVATED,
    ...
    def _on_active_ap_changed(self, device: NM.DeviceWifi, param_spec):
        ...
        if self._access_point:
            ...
            # *** FIXED HERE ***
            ssid = (
                ssid_bytes.get_data().decode("utf-8", errors="replace")
                if ssid_bytes
                else "[Hidden]"
            )
...
class EthernetService(Service):
    ...
    def __init__(self, client, device, **kwargs):
        ...
        if self._device:
            bulk_connect(self._device, {"state-changed": self._on_state_changed})
            # Set initial state
            self._on_state_changed(
                self._device,
                self._device.get_state(),
    ...
    def _on_state_changed(
        ...
        logger.debug(
            f"Ethernet state changed: {old_state.value_nick} -> {new_state.value_nick}"
        )
        if new_state == NM.DeviceState.ACTIVATED:
            self.emit("enabled")
        else:
    ...
    def _get_device(self, device_type: NM.DeviceType) -> Any:
        ...
        return next(
            (
                x
                for x in devices
                if x.get_device_type() == device_type and x.get_managed()
            ),
            None,
        )
...
def on_device_ready(service: NetworkService, *args):
    ...
    if service.wifi_device:
        ...
        service.wifi_device.connect(
            "scan-complete", on_wifi_scan_complete, service.wifi_device
        )  # Pass service for AP list
        service.wifi_device.connect(
            "network-change", on_wifi_network_change, service.wifi_device
        )
