...
class gradientMaker:
    def __init__(self,width=1220,height=62) -> None:
        ...
        self.width = width
        self.height = height
        self.watcher = Watcher(get_relative_path("../styles/colors.css"),self._generate_gradient_image)
        self.watcher.watch()
    ...
    def _generate_gradient_image(self):
        ...
        colors = []
        with open(get_relative_path("../styles/colors.css"),"r",encoding="utf-8") as file:
            data = file.read()
            matches = re.findall(r"#[0-9A-Fa-f]{3}(?:[0-9A-Fa-f]{3})?(?!$)",data,re.MULTILINE)
...
if __name__ == "__main__":
    ...
    monitor_file(get_relative_path("../styles/style.css")).connect("changed",lambda *a: maker._generate_gradient_image())
    app.run()
