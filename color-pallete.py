...
def extract_colors_kmeans(image_path: str, n_colors: int = 16):
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            kmeans = KMeans(n_clusters=n_colors, random_state=42, init='k-means++', n_init=1)
            kmeans.fit(pixels)
...
def display_color_palette(colors, percentages):
    ...
    for i, (color, percentage) in enumerate(zip(colors, percentages)):
        ...
        print(f"Color {i+1}: {bg_color}  {bg_color.OFF} RGB({r},{g},{b}) {hex_color} ({percentage:.1f}%)")
...
if __name__ == "__main__":
    ...
    colors, percentages = extract_colors_kmeans(subprocess.getoutput("swww query | awk -F'image: ' '{print $2}'"), n_colors=8)
    #print(colors)
